	...
	self.events = {
		...
		"cmd#ustalk": function(nick, to, args) {
			...
			if (nick2) {
				...
				for (var i = 0; i < tosort.length && i < 10; i++) {
					var row = tosort[i];
					bot.notice(nick, row.nick + "!" + row.user + "@" + row.host + " seen " + bot.plugins.date.printDateTime(row.seen) + " (" + bot.plugins.date.printDurShort(row.seen, "second", 2) + " ago)");
				}
			}
		},
		...
		"cmd#seen": function(nick, to, args) {
			...
			if (nick2) {
				...
				if (recent !== null && first !== null) {
					bot.say(to, nick2 + " last seen as " + recent.nick + ": " + bot.plugins.date.printDateTime(recent.seen) + " (" + bot.plugins.date.printDurShort(recent.seen, "second", 2) + " ago); first seen as " + first.nick + ": " + bot.plugins.date.printDateTime(first.added) + " (" + bot.plugins.date.printDurShort(first.added, "second", 2) + " ago)");
				}
				else
		...
		"cmd#seen2": function(nick, to, args) {
			...
			if (nick2) {
				...
				if (recent !== null && first !== null) {
					bot.say(to, nick2 + " last seen: " + bot.plugins.date.printDateTime(recent.seen) + " (" + bot.plugins.date.printDurShort(recent.seen, "second", 2) + " ago); first seen: " + bot.plugins.date.printDateTime(first.added) + " (" + bot.plugins.date.printDurShort(first.added, "second", 2) + " ago)");
				}
				else
					bot.say(to, nick2 + " has been never seen");
			}
